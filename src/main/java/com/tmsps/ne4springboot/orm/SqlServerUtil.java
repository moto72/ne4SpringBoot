...
public class SqlServerUtil {
	...
	public static String getSelectByIdSQL(String tableName, String pkName) {
		if (StrUtil.isBlank(pkName)) {// 主键为空则使用约定主键 _id
			...
			log.error("tableName and pkName can not be null !!!");
		}
		return null;
	}
	...
	public static String getSelectByIdSQL(String tableName) {
		if (StrUtil.isNotBlank(tableName)) {
			...
			log.error("tableName can not be null !!!");
		}
		return null;
	}
	...
	public static String getPageSQL(int pageSize,String selectContent,String fromContent,String orderByContent) {
		StringBuilder sb = new StringBuilder();
		sb.append("select top ").append(pageSize).
		append("  * from (select row_number() over(order by ").append(orderByContent)
		.append(") as rownumber,").append(selectContent).append(" from ").append(fromContent)
		.append(" ) A where rownumber > ?");
		return sb.toString();
	}
}
